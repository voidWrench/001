...
parfor t0=1:ite%随机生成ite组信道，每组信道跑一次优化，最后对结果取平均
    ...
    %%
    %Hd=pd.*Hd_w(:,:,t0);
    Hd = pd(:,1:K) .* Hd_w(:,1:K,t0);
    %%
    %排查错误用
    disp('Hd size:')
    disp(size(Hd))

    tempHr = channel_Hr(User_angle,Hr_sig(:,:,t0),eb1,eb2,K,N);
    disp('channel_Hr size:')
    disp(size(tempHr))
    ...
    [ W,grt,f0 ] = init_W( H,M,K,Pt,omega );%初始化BS beamforming，计算初始WSR（f0）
     ...
     flag=0;
     t_old=1;
     [ beta ] =upadte_beta( H,W,K,grt);%这是WMMSE里的辅助变量，把WSR转化为可优化形式
...
figure
plot(0:it,rate_w,'r-');
save('converge_A2_perfect_K8.mat','it','rate_w','M','K','N','omega');
%%
